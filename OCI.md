...
When cloud projects are deployed, their URLs are written to `.env.local`. If the portfolio is cloud-deployed on Azure, the script automatically uploads `.env.local` to the portfolio VM and restarts `npm start` — no rebuild needed. The portfolio VM does not SSH back to project VMs; all communication is one-directional from the local deploy machine.
